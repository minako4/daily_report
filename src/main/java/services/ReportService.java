...
public class ReportService extends ServiceBase {
    ...
    /**
     * フォローした従業員が作成した日報データを、指定されたページ数の一覧画面に表示する分取得しReportViewのリストで返却する
     * @param employee 従業員
     * @param page ページ数
     * @return 一覧画面に表示するデータのリスト
     */
    public List<ReportView> getAllFollow(EmployeeView employee, int page) {

        List<Report> followReports = em.createQuery(JpaConst.Q_REP_GET_ALL_FOLLOW_DEF, Report.class)
    ...
    /**
     * フォローした従業員が作成した日報データの件数を取得し、返却する
     * @param employee
     * @return 日報データの件数
     */
    public long countAllFollow(EmployeeView employee) {

        long count = (long) em.createQuery(JpaConst.Q_REP_COUNT_ALL_FOLLOW_DEF, Long.class)
                .setParameter(JpaConst.JPQL_PARM_FOLLOWEE, EmployeeConverter.toModel(employee))
                .getSingleResult();
